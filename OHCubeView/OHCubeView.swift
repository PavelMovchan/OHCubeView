...
@objc open class OHCubeView: UIScrollView, UIScrollViewDelegate {
    @objc public weak var cubeDelegate: OHCubeViewDelegate?
    ...
    @objc func setScrollRectToVisible(rect _: CGRect) {
        scrollRectToVisible(frame, animated: false)
        transformViewsInScrollView(self)
    }

    @objc open func scrollToViewAtIndexWithDecelerationAnimation(_ index: Int, animated _: Bool, noAnyChanges: Bool, isUserInteraction: Bool) {
        noAnyChangesGlobal = noAnyChanges
        isUserInteractionGlobal = isUserInteraction
        ...
        if index > -1, index < childViews.count {
            ...
            let s = contentOffset.x
            let f = frame.origin.x

            let d = 0.18
            ValueAnimator.frameRate = 90
            let animator = ValueAnimator.animate(props: ["some"], from: [s], to: [f], duration: d, onChanged: { _, v in
    ...
    @objc open func scrollToViewAtIndexWithDecelerationAnimationSlow(_ index: Int, animated _: Bool, noAnyChanges: Bool, isUserInteraction: Bool) {
        noAnyChangesGlobal = noAnyChanges
        isUserInteractionGlobal = isUserInteraction
    ...
    open func scrollViewWillBeginDragging(_: UIScrollView) {
        var location = panGestureRecognizer.location(in: self) // [scrollView.panGestureRecognizer locationInView:scrollView];
        startPoint = location
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        var isr = false
        if lastContentOffset > Double(scrollView.contentOffset.x) {
            isr = true
        } else if lastContentOffset < Double(scrollView.contentOffset.x) {
            isr = false
        }
        ...
        lastContentOffset = Double(scrollView.contentOffset.x)
    ...
    fileprivate func applyShadowForView(_ view: UIView, index: Int) {
        ...
        let r1 = frameFor(origin: contentOffset, size: frame.size)
        let r2 = frameFor(origin: CGPoint(x: CGFloat(index) * w, y: 0), size: frame.size)
        func block() {
            ...
            let multiplier = 1.0
            let opac = ((intersection.size.width * 0.75) / frame.size.width) + 0.25
